...
export default function Dashboard() {
  ...
  // Student Dashboard
  const quickActions = [
    { path: '/chats',      icon: 'chat',   label: 'Open chat',        sub: '20 unread',      iconSvg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg> },
    { path: '/internships', icon: 'intern', label: '5 new internships', sub: '92% match',     iconSvg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/></svg> },
    { path: '/mentorship',  icon: 'mentor', label: 'Book a mentor',    sub: 'Free slots today', iconSvg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> },
  ];
    ...
    <>
      ...
      <div className="welcome-block">
        <div>
          ...
          <p className="welcome-meta">{user.branch} &nbsp;·&nbsp; Semester {user.semester} &nbsp;·&nbsp; Roll {user.roll}</p>
        </div>
      </div>
      ...
      <div className="stats-row" role="list" aria-label="Academic statistics">
        <div className="stat-card" role="listitem">
          ...
          <span className={`stat-delta ${user.cgpaDeltaClass || 'positive'}`}>{user.cgpaDelta}</span>
        </div>
        <div className="stat-card" role="listitem">
          <span className="stat-label">Attendance</span>
          <span className="stat-value">{user.attendance}</span>
          <span className="stat-delta neutral">Min. required 75%</span>
        </div>
        <div className="stat-card" role="listitem">
          <span className="stat-label">Pending tasks</span>
          <span className="stat-value">{user.pendingTasks}</span>
          <span className="stat-delta warning">{user.tasksDelta}</span>
        </div>
        <div className="stat-card" role="listitem">
          <span className="stat-label">Class rank</span>
          <span className="stat-value">{user.classRank}</span>
          <span className="stat-delta neutral">of {user.totalStudents} students</span>
        </div>
      </div>
      ...
      <div className="mid-row">
        ...
        <section className="panel insight-panel" aria-labelledby="insightTitle">
          ...
          <h3 className="insight-title" id="insightTitle">Attendance dipped in November</h3>
          <p className="insight-body">Theory of computation is at 77%. Three more missed classes will lock you out.</p>
          <button
