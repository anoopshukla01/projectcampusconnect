...
def register_student():
    ...
    # Check pre-imported StudentProfile by roll_no
    profile = db.session.query(StudentProfile).filter_by(roll_no=data["roll_no"], is_deleted=False).first()
    if not profile:
        return error_response("Student record not found in college registry. Please ask your administrator to import your record.", 403)
    ...
    try:
        ...
        if not user:
            user = User(phone=phone, role=UserRole.STUDENT, is_active=True)
            db.session.add(user)
            db.session.flush()
...
def invite_accept():
    ...
    try:
        ...
        user = User(email=invite.email, role=role, is_active=True)
        user.set_password(data["password"])
        db.session.add(user)
        db.session.flush()
