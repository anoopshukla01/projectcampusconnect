...
export default function Notes() {
  ...
  const { data: apiData, loading, error, isEmpty } = useApiData('/api/v1/career/notes', { notes: [] });
  ...
  const filtered = useMemo(() => {
    return type==='all' ? notes : notes.filter(n=>n.type===type);
  }, [notes, type]);
  ...
  return (
    <>
      ...
      <StateContainer loading={loading} error={error} isEmpty={isEmpty} emptyMessage="No study resources uploaded yet. Be the first to share one!">
        <div className="notes-list">
          {filtered.map(n=>(
            <div className="note-card" key={n.id}>
              ...
              <div className="note-body">
                ...
                <p className="note-meta">by {n.uploader} {n.size ? `· ${n.size}` : ''} {n.date ? `· ${n.date}` : ''}</p>
              </div>
              <span className={`note-type-badge ${n.type}`}>{TYPE_LABELS[n.type] || 'Notes'}</span>
