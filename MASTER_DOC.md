...
- **Attendance Roster Security & Active Class Time Lock**: Restricted professor roster fetching (`GET /academics/roster`) and attendance marking (`POST /academics/attendance/mark`) strictly to the professor's currently active `TimetableSlot` (within `[start - 15min, end + 2h]`) and cross-checked with active `ProfessorClassAssignment`. Eliminated all free-form branch/semester/subject inputs for the professor role in `Attendance.jsx` UI and backend (derives course fields server-side). Added `GET /academics/roster/active-class` endpoint, double-booking candidate selection (409 Conflict), and added `college_id` column to `TimetableSlot` model with migration `m8n9o0p1q2r3_add_college_id_to_timetable_slots.py`. Added test suite `test_attendance_roster_security.py`. All 88 tests pass.
- **"New Assignment" form class picker & assignment multi-tenancy**: Replaced free-text Subject and Branch inputs in 'New Assignment' modal (`Assignments.jsx`) with a class picker dropdown populated via `GET /professors/me/classes`. Displays inline warning (*"You have no classes assigned yet — contact your Admin"*) and disables submit button if professor has zero assigned classes. Updated `Assignment` model in `academic.py` and Alembic migration `l7m8n9o0p1q2_add_semester_and_college_id_to_assignments.py` to add `semester` and `college_id` (nullable=False, indexed). Enforced class ownership server-side in `POST /academics/assignments` against `ProfessorClassAssignment` (returns 403 on mismatch) and scoped all assignment list queries to `college_id == user.college_id`. Added pytest test suite in `test_assignments.py`. All 81 tests pass.
